...
def add_fusion_generation(
    ...
    n.madd(
        ...
        capital_cost=costs.at["fusion", "fixed"],  # NB: fixed cost is per MWel
        marginal_cost=costs.at["fusion", "VOM"], # NB: VOM is per MWel,
        unit="MWh_el",
...
# antbau
# add fusion as a generator of electricity and heat
# done by converting the add_fusion_generation to add fusion as a Link instead of a generator
def add_fusion_generation_heat(
        n,
        costs
):
    logger.info("Adding fusion generation with heat.")

    nodes = n.buses.index[n.buses.carrier == "AC"]
    n.add("Carrier", "fusion")
    n.add("Carrier", "deu_tri")

    # add fusion fuel bus
    n.add("Bus", "fusion fuel bus", location="EU", carrier="deu_tri", unit="MWh_th")

    n.add(
        "Generator",
        nodes + " fusion fuel generator",
        bus="fusion fuel bus",
        carrier="deu_tri",
        p_nom_extendable=True,
    )
    ...
    for node in nodes:
        ...
        n.add(
            "Link", # implemented as a link, as fusion has to connect to two buses: electricity and heat
            f"{node} fusion", # name of the new type of node
            bus0= "fusion fuel bus", # connects the fusion bus to the fusion fuel
            bus1=node, # connects the fusion bus to the electricity network
            bus2=heat_bus, # connects the fusion bus to the heat network TODO: this might have to be adjusted
            carrier="fusion", # newly defined carrier for fusion
            p_nom_extendable=True, # sets capacity expansion to true, hence capacity expansion is optimized
            capital_cost=costs.at["fusion", "fixed"] * costs.at["fusion", "efficiency"],  # NB: fixed cost is per MWel
            marginal_cost=costs.at["fusion", "VOM"] * costs.at["fusion", "efficiency"],  # NB: VOM is per MWel,
            efficiency=costs.at["fusion", "efficiency"]*0.85, # efficiency of the fusion plant for electricity multiplied by 0.85 (efficiency reduction through cogeneration)
            efficiency2=costs.at["fusion", "efficiency"]*0.15+0.04, # overall efficiency of the plant increased by 4% due to cogeneration
            lifetime=costs.at["fusion", "lifetime"],
        )
...
if __name__ == "__main__":
    ...
    # antbau: added option for heat generation
    if snakemake.params.fusion_inclusion:
        ...
        if snakemake.params.fusion_entry_year <= int(current_horizon):
            if snakemake.params.fusion_heat:
                add_fusion_generation_heat(n, costs)
            else:
