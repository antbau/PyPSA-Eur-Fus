...
#antbau
# calculate discounted cumulative costs per node
def calculate_cumulative_cost_per_node():
    planning_horizons = snakemake.params.scenario["planning_horizons"]
    social_discount_rate = [0, 0.02, 0.05]

    # Initialize DataFrame to store cumulative costs
    cumulative_cost = pd.DataFrame(
        index=pd.MultiIndex.from_product(
            [
                df["nodal_costs"].sum().index.get_level_values(level=0).unique(),  # cluster
                df["nodal_costs"].sum().index.get_level_values(level=1).unique(),  # ll
                df["nodal_costs"].sum().index.get_level_values(level=2).unique(),  # opt
                df["nodal_costs"].index.get_level_values(level=2).unique(),        # node
                planning_horizons
            ],
            names=["cluster", "ll", "opt", "node", "planning_horizon"]
        ),
        columns=pd.Series(data=social_discount_rate, name="social discount rate")
    )

    # discount cost and express them in money value of planning_horizons[0]
    for idx in cumulative_cost.index:
        cluster, ll, opt, node, year = idx

        for r in social_discount_rate:
            col_filter = (
                (df["nodal_costs"].columns.get_level_values(level=0) == cluster) &
                (df["nodal_costs"].columns.get_level_values(level=1) == ll) &
                (df["nodal_costs"].columns.get_level_values(level=2) == opt) &
                (df["nodal_costs"].columns.get_level_values(level=3) == year)
            )

            node_costs = df["nodal_costs"].loc[
                df["nodal_costs"].index.get_level_values(level=2) == node, col_filter
            ]

            if not node_costs.empty:
                discounted_cost = node_costs.sum() / ((1 + r) ** (year - planning_horizons[0]))
                cumulative_cost.loc[idx, r] = discounted_cost.iloc[0]
        ...
        for cluster in cumulative_cost.index.get_level_values(level=0).unique():
            for ll in cumulative_cost.index.get_level_values(level=1).unique():
                for sector_opts in cumulative_cost.index.get_level_values(level=2).unique():
                    for node in cumulative_cost.index.get_level_values(level=3).unique():
                        loc_index = (cluster, ll, sector_opts, node, planning_horizons)
                        cumulative_cost.loc[
                            (cluster, ll, sector_opts, node, "cumulative cost"), r
                        ] = np.trapz(
                            cumulative_cost.loc[loc_index, r].values,
                            x=planning_horizons
                        )
...
if __name__ == "__main__":
    ...
    if snakemake.params.foresight == "myopic":
        cumulative_energy_cost = calculate_cumulative_cost_per_node()
        cumulative_energy_cost.to_csv(
            "results/" + snakemake.params.RDIR + "csvs/cumulative_costs_per_node.csv"
        )
